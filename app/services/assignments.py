...
class AssignmentServices:
    ...
    def get_project_assignments(
        ...
        try:
            ...
            logger.info(
                f"Получено {len(response.data)} заданий"
                f"для проекта {project_id}"
            )
    ...
    def create_assignment(
        ...
        try:
            ...
            if not from_section.data:
                logger.info(f"Секция с ID {assignment.from_section_id} "
                            f"не найдена")
            ...
            if not to_section.data:
                logger.info(f"Секция с ID {assignment.to_section_id} "
                            f"не найдена")
            ...
            if not response.data:
                raise AssignmentDatabaseError("создании задания",
                                              "Нет данных в ответе")
    ...
    def update_assignment_status(
        ...
        try:
            ...
            logger.info(
                f"Обновлен статус задания с ID {assignment_id} "
                f"на {status_update.status}"
            )
